  ...
  <div>
    ...
    >
        ...
        <template v-else>
          ...
          >
            ...
            >
              ...
              <template v-else>
                <div v-bkloading="{ isLoading: item.isScrollRemote, size: 'mini' }" />
              </template>
            </bk-option>
          </bk-select>
        </template>
      ...
      <div class="attribute-action" v-if="isShowAction(item)">
        <Icon type="add-hollow" @click="handleAddAttribute" />
        <Icon
          type="reduce-hollow"
      ...
      value: {
        handler (val) {
          ...
          this.attrValues.forEach(async item => {
            if (!flag && item.id) {
              await this.fetchAttributeValue(item);
            }
          });
        },
        immediate: true
        ...
        try {
          ...
          const results = (() => {
            ...
            if (this.isDisabledMode && limitItem) {
              return data.results.filter(v => limitItem.selecteds.includes(v.id));
            }
            return data.results;
          })();
